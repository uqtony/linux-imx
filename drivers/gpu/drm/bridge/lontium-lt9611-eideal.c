				     ...
				     const struct drm_display_mode *mode)
{
	...
	dev_info(lt9611->dev, "[%s]Use PortB\n", __func__);
...
static void lt9611_copy_drm_display_mode(struct lt9611 *lt9611,
                                    const struct drm_display_mode *mode)
{
	lt9611->mode.htotal = mode->htotal;
        lt9611->mode.vtotal = mode->vtotal;

        lt9611->mode.hdisplay = mode->hdisplay;
        lt9611->mode.vdisplay = mode->vdisplay;

        lt9611->mode.hsync_start = mode->hsync_start;
        lt9611->mode.hsync_end = mode->hsync_end;

        lt9611->mode.vsync_start = mode->vsync_start;
        lt9611->mode.vsync_end = mode->vsync_end;
}
				    ...
				    const struct drm_display_mode *mode)
{
	...
	dev_info(lt9611->dev, "[%s][h_total, v_total]=[%u, %u]\n", __func__, h_total, v_total);
	dev_info(lt9611->dev, "[%s][h_active, v_active]=[%u, %u]\n", __func__, hactive, vactive);
	dev_info(lt9611->dev, "[%s][hsync_len, hfront_porch, hsync_porch]=[%u, %u, %u]\n", __func__, hsync_len, hfront_porch, hsync_porch);
	dev_info(lt9611->dev, "[%s][vsync_len, vfront_porch, vsync_porch]=[%u, %u, %u]\n", __func__, vsync_len, vfront_porch, vsync_porch);
...
static int lt9611_pll_lock_test(struct lt9611 * lt9611)
{
	int loop_index = 0;
	unsigned int cal_done_flag, band_out, pll_lock_flag;

	for (loop_index  = 0; loop_index < 5; loop_index++) {
		regmap_write(lt9611->regmap, 0x8016, 0xe3);
		regmap_write(lt9611->regmap, 0x8016, 0xf3); /* pll lock logic reset */
		...
		pll_lock_flag &= 0x80;
		cal_done_flag &= 0x80;
		if (pll_lock_flag && cal_done_flag && (band_out != 0xff)){
			dev_info(lt9611->dev, "[%s]HDMI pll locked, band out=0x%x\n", __func__, band_out);
			return 0;
		}
		...
		dev_info(lt9611->dev, "[%s]HDMI pll unlocked, reset pll\n", __func__);
...
static void lt9611_hdmi_out_enable(struct lt9611 *lt9611)
{
	struct reg_sequence reg_cfg[] = {
		...
		{ 0x82de, 0x20 },
		{ 0x82de, 0xe0 },
		{ 0x8018, 0xdc }, /* txpll sw reset */
		{ 0x8018, 0xfc },
		{ 0x8016, 0xf1 }, /* txpll calibration reset */
		{ 0x8016, 0xf3 },
		{ 0x8011, 0x5a }, /* pcr reset */
		{ 0x8011, 0xfa },
		{ 0x8130, 0xea },
	};
...
// lt9611 debug function
static void lt9611_frequency_meter_byte_clk(struct lt9611 *lt9611)
{
	unsigned int temp;
	unsigned int clock_rate;

	regmap_write(lt9611->regmap, 0x82c7, 0x03); // portA
	msleep(50);
	regmap_read(lt9611->regmap, 0x82cd, &temp);
	if ((temp & 0x60) == 0x60) { // clock stable
		clock_rate = (temp & 0x0f) << 16;
		regmap_read(lt9611->regmap, 0x82ce, &temp);
		clock_rate = clock_rate + (temp << 8);
		regmap_read(lt9611->regmap, 0x82cf, &temp);
		clock_rate = clock_rate + temp;
		dev_info(lt9611->dev, "[%s]Port A# byte clock=%u\n", __func__, clock_rate);
	}
	else
	{
		dev_info(lt9611->dev, "[%s]Port A# byte clock unstable\n", __func__);
	}

        regmap_write(lt9611->regmap, 0x82c7, 0x04); // portB
        msleep(50);
        regmap_read(lt9611->regmap, 0x82cd, &temp);
        if ((temp & 0x60) == 0x60) { // clock stable
                clock_rate = (temp & 0x0f) << 16;
                ...
                regmap_read(lt9611->regmap, 0x82cf, &temp);
                clock_rate = clock_rate + temp;
                dev_info(lt9611->dev, "[%s]Port B# byte clock=%u\n", __func__, clock_rate);
        }else
        {
                dev_info(lt9611->dev, "[%s]Port B# byte clock unstable\n", __func__);
        }
}

void lt9611_print_htotal_sysclk(struct lt9611 *lt9611)
{
	unsigned temp = 0;
	...
	for (index = 0; index < 10; index++){
		regmap_read(lt9611->regmap, 0x8286, &temp);
		clock_rate = (temp << 8);
		regmap_read(lt9611->regmap, 0x8287, &temp);
		clock_rate += temp;
                dev_info(lt9611->dev, "[%s]Htotal sys clock=%u\n", __func__, clock_rate);
	}
}

void lt9611_print_pcr_mk(struct lt9611 *lt9611)
{
	unsigned temp = 0;
        int index = 0;
	for (index = 0; index < 10; index++) {
		regmap_read(lt9611->regmap, 0x8397, &temp);
                dev_info(lt9611->dev, "[%s]PCR stable bit[4:0]=0x%xx\n", __func__, temp);
		regmap_read(lt9611->regmap, 0x83b4, &temp);
                dev_info(lt9611->dev, "[%s]\t0x%xx\n", __func__, temp);
		regmap_read(lt9611->regmap, 0x83b5, &temp);
                dev_info(lt9611->dev, "[%s]\t0x%xx\n", __func__, temp);
		regmap_read(lt9611->regmap, 0x83b6, &temp);
                dev_info(lt9611->dev, "[%s]\t0x%xx\n", __func__, temp);
		regmap_read(lt9611->regmap, 0x83b7, &temp);
                dev_info(lt9611->dev, "[%s]\t0x%xx\n", __func__, temp);
		msleep(1000);
	}
}

void lt9611_debug_video_check(struct lt9611 *lt9611)
{
	int index = 0;
	for (index = 0; index < 10; index++) {
...
static void lt9611_full_init(struct lt9611 *lt9611);

static void lt9611_work_handler(struct work_struct *data)
{
        struct lt9611 *lt9611 = container_of(data, struct lt9611, delayed_work.work);

	lt9611_full_init(lt9611);
	lt9611_pll_lock_test(lt9611);
	lt9611_frequency_meter_byte_clk(lt9611);
	lt9611_debug_video_check(lt9611);
	lt9611_print_htotal_sysclk(lt9611);
	lt9611_print_pcr_mk(lt9611);
}

static int lt9611_init_work(struct lt9611 *lt9611)
{
        INIT_DELAYED_WORK(&lt9611->delayed_work, lt9611_work_handler);
        return 0;
}
...
static void lt9611_full_init(struct lt9611 *lt9611)
{
	...
	for (loop_index = 0; loop_index < 20; loop_index++) {
        ...
        lt9611_mipi_input_digital(lt9611, &lt9611->mode);
        lt9611_pll_setup(lt9611, &lt9611->mode);
        lt9611_mipi_video_setup(lt9611, &lt9611->mode);
        lt9611_pcr_setup(lt9611, &lt9611->mode);
...
static void lt9611_bridge_enable(struct drm_bridge *bridge)
{
	...
	lt9611_hdmi_out_enable(lt9611);
	schedule_delayed_work(&lt9611->delayed_work, msecs_to_jiffies(10));
}
				   ...
				   const struct drm_display_mode *adj_mode)
{
	...
	lt9611_copy_drm_display_mode(lt9611, mode);
